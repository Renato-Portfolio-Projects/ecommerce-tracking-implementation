...
The currency function was also called on a real Vercel Preview, which found two faults the local server had hidden. The function did not start, because Vercel runs it without bundling and Node then needs `.js` on every import. And the page's request was sent to Vercel's sign-in page, because it had asked to leave cookies out. Both are fixed, and each now has a test. On the Preview the function answers, refuses a POST, is never cached, and reports Canada for this connection, which a missing country would not have done. A country header sent by a client is ignored, so the country cannot be faked. The page asked once, kept the answer for the tab, and logged no error.

Not checked, and so not claimed: real phones, Safari and Firefox (the whole checking was done in Chrome, using its phone emulation), a real screen reader, and any country but Canada on a real deployment. Those were checked against the local server only, which stands in for Vercel's country header. Labels, focus order and announcements were checked by script, which is not the same as hearing them.
