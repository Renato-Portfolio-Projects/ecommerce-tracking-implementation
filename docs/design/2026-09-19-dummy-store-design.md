...
Opens by itself on the home page only, so never on checkout or thank-you, after 5 seconds or once the visitor has scrolled 40% of the way down (of the distance the page can be scrolled), whichever comes first, at most once every 7 days, and never over another dialog. The seconds count only while the tab is on screen, and they start again when a dialog such as the cart drawer closes. The 5 seconds is a deliberate demo setting, so a reviewer sees the popup quickly. A link in the footer and a small corner tab, on every store page, open the same form by hand, at any time. It is a native dialog, so focus is trapped and Esc closes it. It also closes with the X, "No thanks" and a click on the dimmed page, which is ignored once something has been typed. A half-typed form is kept when it is closed, and the form is put back empty once the code has been taken. Fields: first name, email, unticked marketing consent. Success state shows the code. The server checks the email before it saves the lead and before `generate_lead` fires: its shape, that its domain is not a temporary-email service, and that the domain can receive mail. So junk addresses never become conversions. Only the domain is looked up, and no email address is sent to any other service. The demo domains (`example.com`, `example.org` and `example.net`) are accepted without a mail check, so the demo people work. See `docs/shop-rules.md`.
...
- One fictional persona per session, picked at random on first use and reused everywhere. The lead and the order then share an identity, so `/proof` can link the lead to the sale. A "Try another person" button, shown once a person has been filled in, re-rolls it, always to a different one. There are eight, and none of them has a consent setting.
