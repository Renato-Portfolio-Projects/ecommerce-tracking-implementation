...
- **The lead popup offers three announcements and three `data-cta` labels to tag against (built in v0.2b-4, and sending nothing yet).** On the page it announces `lead-popup:shown` (with `source`: `auto` or `manual`), `lead-popup:closed` (with `reason`: `close-button`, `no-thanks`, `escape`, `backdrop` or `other`) and `lead-popup:submitted` (with the checked first name and email, whether the marketing box was ticked, and the code). `lead-popup:shown` is the moment for `view_promotion`. `lead-popup:submitted` is the moment for `generate_lead`, but only once the server has said OK (v0.2c), and because it carries the address as typed, the email must be hashed before anything reaches the dataLayer. Whether `source` is sent to GA4 is decided in v0.2d. The `cta_click` labels are `Lead Popup - Footer link`, `Lead Popup - Corner tab` and `Demo Data - Lead popup`. "Try another person" has no label, because it is a convenience and not something the business wants to count.
