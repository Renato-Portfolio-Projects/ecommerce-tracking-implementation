...
The functions themselves are the files in the `api/` folder at the top of the repository. Each is two lines that hand a request to a handler in `src/server`, and a test says so, so that no logic sits where the tests cannot reach it. Vercel runs each file as a function at `/api/` and the file's name. It runs them as native modules, file by file, without bundling, so every import in a function, and in every file the function reaches, names its file with `.js` (`'./gate.js'`, which TypeScript, Vite and esbuild all read as `gate.ts`). Leaving the extension off works everywhere except on Vercel, where the function then fails to start. A test runs each function that way. To try them on this machine, `npm run serve:store` serves the built store and the functions together; it bundles them, so it would not notice.
...
| Which drawing a product uses | Every colour of a product is the same drawing recoloured, so there is one drawing to change | `ART` in `src/store/art.ts` |
