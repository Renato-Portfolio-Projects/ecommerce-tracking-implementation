...
Sources checked on 2026-09-21. A row added after that day gives, in its source column, the day its source was checked or the day the source page says it was updated.
...
| **Prices come from the server.** Planned (v0.2c): the server prices every order again from the catalog. The pricing code it will run is written and tested, and the cart holds no prices. The cart's own screens show prices worked out in the browser by that same code, as a preview; the amount to be charged is the server's. | Keep it. A price that came from the browser must never be trusted, for the same reason that server-side checks are the ones that count: anything in the browser can be changed. | `src/engine/pricing.ts`, `src/engine/cart.ts`, `src/engine/cart-view.ts` | [OWASP: Input Validation Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html) |
| **Where the cart lives.** In the shopper's browser (`localStorage`), for 7 days from its last change, on that browser and device only. It holds no prices. The cart's lines are priced and drawn in the browser. | A cart that the store's own platform or server keeps and creates, which the browser refers to by an ID, so the cart is priced with the store's live prices and estimates, expires after a period the store chooses, and can be cleared when the order is placed. Shopify's Storefront API works this way: `cartCreate` makes the cart, its ID has two parts, a token and a secret key, that the storefront must send in full with every request, unused carts expire within 30 days of creation, and the cart is deleted when checkout completes. The browser then only shows what the server sends. | `src/components/cart-client.ts`, `src/engine/cart-storage.ts`, `src/engine/cart-view.ts` | [Shopify: cart in the Storefront API](https://shopify.dev/docs/storefronts/headless/building-with-the-storefront-api/cart) and [Shopify: create and update a cart](https://shopify.dev/docs/storefronts/headless/building-with-the-storefront-api/cart/manage) (neither page shows a date; checked 2026-09-24) |
| **Sold-out sizes.** Which colours and sizes are sold out is a list in the catalog. It is written into each product page when the site is built, so the size picker can switch those sizes off, and the cart and pricing code refuse a sold-out item too. Nothing is held for a shopper while they decide. | Live stock, read when it counts. A page built ahead of time can go on showing a size as available after it has sold out. Shopify, for example, does not let a customer buy an out-of-stock item unless the merchant turns on "Continue selling when out of stock", and reserves the units for a short time once payment starts, so that two checkouts cannot claim the same one. | `src/store/products.ts`, `src/engine/catalog.ts`, `src/store-pages/products/product-picker.ts` | [Shopify Help Center: selling out-of-stock products](https://help.shopify.com/en/manual/products/inventory/setup/selling-when-out-of-stock) (no date shown; checked 2026-09-24) and [Shopify Engineering: scaling inventory reservations](https://shopify.engineering/scaling-inventory-reservations) (published 2026-05-12) |
